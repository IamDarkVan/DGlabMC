...
public class CommandScript extends CommandAbstract {
    ...
    protected void errorHandle() throws CommandException {
        if (length == 3) {
            ...
            if (getScriptManager().getScript(args[1]) == null) throw new CommandException("未找到该游戏");
            this.client = getClient(player);
            this.script = getScriptManager().getScript(args[1]);
            this.type = args[2];
        }
        if (length == 4) {
            if (!isClientExist(args[1]) && !isClientExist(getPlayer(args[1]))) throw new CommandException("客户端不存在或玩家未绑定");
            if (getScriptManager().getScript(args[2]) == null) throw new CommandException("未找到该游戏");
            this.client = isClientExist(args[1]) ? getClient(args[1]) : getClient(getPlayer(args[1]));
            this.script = getScriptManager().getScript(args[2]);
            this.type = args[3];
        }
    ...
    public List<String> tabComplete() {
        if (length == 2) return concatList(CommandUtils.getPlayerAndClientList(sender), getScriptManager().getNameSet().toArray(new String[0]));
        if (getPlayer(args[1]) != null || isClientExist(args[1])) {
            if (length == 3) return new ArrayList<>(getScriptManager().getNameSet());
